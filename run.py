...
def run_Generatefsf_processing(**args):
    ...
    cmd = '/home/range2-raid1/timothy/GitHub/BIDS_hcp_rsfMRI_seed_analysis/modified_files/generate_level1_fsf.sh ' + \
        ...
        '--outdir="{path}/{subject}/{session}/MNINonLinear/Results/{fmriname}" '
    cmd = cmd.format(**args)
    run(cmd, cwd=args["path"])
    pdb.set_trace()
    fsf_file = "{path}/{subject}/{session}/MNINonLinear/Results/{fmriname}/{subject}_{session}_{shortfmriname}_hp200_s2_level1.fsf"
    fsf_file = fsf_file.format(**args)
    
    regressor_file = "{regressor_file}"
    regressor_file = regressor_file.format(**args)
...
def run_create_seed_regressor_processing(**args):
    args.update(os.environ)
    #cmd = '/rsfMRI_seed.py ' + \
    cmd = '/home/range2-raid1/timothy/GitHub/BIDS_hcp_rsfMRI_seed_analysis/modified_files/rsfMRI_seed.py ' + \
    ...
    cmd = cmd.format(**args)
    # TODO: test to determine whether we an get regressor_file from x
    x = run(cmd, cwd=args["path"])
