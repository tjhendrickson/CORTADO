...
#separate optional and required arguments
optional = parser._action_groups.pop()
required = parser.add_argument_group('required arguments')
required.add_argument('--input_dir', help='The directory where the preprocessed derivative needed live',required=True)
required.add_argument('--output_dir', help='The directory where the output files should be stored.',required=True)
required.add_argument('--group', help='Whether to run this participant by participant or the entire group. Choices are "participant" or "batch". If participant by participant "--participant_label" and "--session_label" must be specified',choices = ['participant','batch'],required=True)
optional.add_argument('--participant_label', help='The label of the participant that should be analyzed. The label '
                   'corresponds to sub-<participant_label> from the BIDS spec '
                   '(so it does not include "sub-"). If this parameter is not '
                   'provided all subjects should be analyzed. Multiple '
                   'participants can be specified with a space separated list.',
                   nargs="+")
optional.add_argument('--session_label', help='The label of the session that should be analyzed. The label '
                   'corresponds to ses-<session_label> from the BIDS spec '
                   '(so it does not include "ses-"). If this parameter is not '
                   'provided all sessions within a subject should be analyzed.',
                   nargs="+")
optional.add_argument('--preprocessing_type', help='BIDS-apps preprocessing pipeline run on data. Choices include "HCP" and "fmriprep". Default: "HCP".',choices=['HCP','fmriprep'],default='HCP')
optional.add_argument('--use_ICA_outputs',help='Use ICA (whether FIX or AROMA) outputs in seed analysis. Choices include "Y/yes" or "N/no". Default: "Yes".',choices=['Yes','yes','No','no'],default='Yes')
optional.add_argument('--combine_resting_scans',help='If multiple of the same resting state BIDS file type exist should they be combined prior seed analysis? Choices include "Y/yes" or "N/no". Default "No".',choices=['Yes','yes','No','no'],default='No')
optional.add_argument('--smoothing',help="What FWHM smoothing (in mm) to apply to final output, only applies to SBA with dtseries. Default: 4",default=4, type=int)
required.add_argument('--parcellation_file', help='The CIFTI label/atlas file to use or used to parcellate the brain. ', required=True)
required.add_argument('--parcellation_name', help='Shorthand name of the CIFTI label file. ', required=True)
required.add_argument('--seed_ROI_name', help='Space separated list of ROI name/s from CIFTI label file to be used as the seed ROI/s. The exact ROI from the label file must be known!', nargs="+",required=True)
optional.add_argument('--seed_handling', help='Of the ROI/s you have provided do you want to treat them as together (i.e. averaging ROIs together), or separate (run separate seed based analyses for each ROI)? '
                                        'Choices are "together", or "separate". Default: "separate".',
                                        choices=['together', 'separate'],
                                        default='separate')
optional.add_argument('--seed_analysis_output',help='The output of the seed based analysis. Choices are "dense" (i.e. dtseries.nii) and "parcellated" (i.e. ptseries.nii)). Default: "parcellated".',choices = ['dense','parcellated'], default = 'parcellated')
optional.add_argument('--motion_confounds',help='What type of motion confounds to use, if any. Note only works in combination with "--statistic regression". Choices are "Movement_Regressors" (motion rotation angles and translations in mm), '
...
optional.add_argument('--reg_name',help='What type of registration do you want to use? Choices are "MSMAll_2_d40_WRN" and "NONE". Default: "MSMAll_2_d40_WRN".',choices = ['NONE','MSMAll_2_d40_WRN'],default='MSMAll_2_d40_WRN')
optional.add_argument('--text_output_format',help='What format should the text output be in? Choices are "CSV" or "NONE". Default: "CSV".', choices=['CSV',"csv",'none','NONE'],default='CSV')
optional.add_argument('--num_cpus', help='How many concurrent CPUs to use. Default: "1".',default=1)
optional.add_argument('--statistic', help='Strategy to calculate functional connectivity. ' 
                    'Choices are "correlation", and "regression" Default: "correlation"', 
                    choices=['correlation','regression'],default='correlation')
# global variables
